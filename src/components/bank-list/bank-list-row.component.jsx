...
const BankListRow = ({bank})=>{
    const {bank_name,ifsc,branch,bank_id,address,favorite,city} = bank;
    return(
        <Table.Row textAlign="center">
           ...
           <Table.Cell width={3} data-city={`${city}`} data-bank={`${ifsc}`}>{branch}</Table.Cell>
           <Table.Cell width={3} data-city={`${city}`} data-bank={`${ifsc}`}>{bank_id}</Table.Cell>
           <Table.Cell width={3} data-city={`${city}`} data-bank={`${ifsc}`}>{address}</Table.Cell>
        </Table.Row>
    )
}
